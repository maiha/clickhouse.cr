...
# ```console
# mkdir -p tmp
# curl -o tmp/ErrorCodes.cpp https://raw.githubusercontent.com/yandex/ClickHouse/master/dbms/src/Common/ErrorCodes.cpp
# grep -oP '(?<=extern const int).+(?=;)' tmp/ErrorCodes.cpp
# ```

enum Clickhouse::ErrorCode
